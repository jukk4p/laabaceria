...
export default function PageHero({ 
  ...
  return (
    <section className="relative pt-36 pb-20 px-6 bg-bg-base overflow-hidden border-b border-gold/10">
      ...
      <div className="container mx-auto max-w-4xl relative z-10 text-center">
        ...
        {titleId ? (
          <AdminEditable id={titleId} content={title}>
            <h1 className="text-4xl md:text-6xl font-serif text-text-primary mb-6 leading-tight opacity-0 animate-fade-in-up">
              {title}
            </h1>
          </AdminEditable>
        ) : (
          ...
          <div className="opacity-0 animate-fade-in [animation-delay:0.3s] [animation-fill-mode:forwards]">
            {subtitleId ? (
              <AdminEditable id={subtitleId} content={subtitle}>
                <p className="text-text-muted text-lg md:text-xl max-w-2xl mx-auto leading-relaxed font-light">
                  {subtitle}
                </p>
              </AdminEditable>
            ) : (
