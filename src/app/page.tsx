    ...
    <div className="flex flex-col bg-bg-base">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center pt-28 pb-16 overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          <AdminEditable id="home_hero_image" content={content['home_hero_image'] || "/images/Jamon_oro.jpg"} type="image" className="w-full h-full block relative">
            <Image
              src={content['home_hero_image'] || "/images/Jamon_oro.jpg"}
              alt="Fondo Gourmet"
              fill
        ...
        <div className="container mx-auto px-6 relative z-10 text-center">
          <div className="mb-10 flex justify-center opacity-0 animate-[fade-in_1s_ease-out_forwards]">
            <AdminEditable id="home_hero_subtitle" content={content['home_hero_subtitle'] || "JAMONES Y EMBUTIDOS · CORIA DEL RÍO · SEVILLA"}>
              <span className="inline-block text-[10px] md:text-[11px] uppercase tracking-[0.5em] text-gold/80 border-b border-gold/20 pb-2 font-bold">
                {content['home_hero_subtitle'] || "JAMONES Y EMBUTIDOS · CORIA DEL RÍO · SEVILLA"}
              </span>
            </AdminEditable>
          </div>
          ...
          <AdminEditable id="home_hero_title" content={content['home_hero_title'] || "El arte del |jamón ibérico|"} className="w-full">
            <h1 className="text-6xl md:text-[8rem] font-serif text-text-primary leading-[1.1] mb-10 max-w-5xl mx-auto opacity-0 animate-[fade-in-up_1s_ease-out_0.3s_forwards]">
              {(() => {
                ...
                // Si el usuario usa el separador |, lo respetamos
                if (text.includes('|')) {
                  const parts = text.split('|');
                  return (
                    <>
                      {parts[0]} <span className="text-gold italic font-light">{parts[1]}</span>
                      {parts[2] || ''}
                    </>
                  );
                }

                // Si no hay |, aplicamos el estilo a las últimas dos palabras automáticamente
                ...
                if (words.length > 2) {
                  ...
                  return (
                    <>
                      {firstPart} <span className="text-gold italic font-light">{highlightPart}</span>
                    </>
                  );
                }
          ...
          <AdminEditable id="home_hero_description" content={content['home_hero_description'] || "Selección artesanal de los mejores embutidos y productos gourmet de nuestra tierra. Tradición y sabor en cada bocado."} className="w-full">
            <p className="text-base md:text-xl text-text-muted mb-14 leading-relaxed max-w-2xl mx-auto font-light opacity-0 animate-[fade-in_1s_ease-out_0.6s_forwards]">
              {content['home_hero_description'] || "Selección artesanal de los mejores embutidos y productos gourmet de nuestra tierra. Tradición y sabor en cada bocado."}
            </p>
          </AdminEditable>

          <div className="flex flex-wrap justify-center gap-8 mb-28 opacity-0 animate-[fade-in_1s_ease-out_0.9s_forwards]">
            <AdminEditable id="home_hero_primary_text" content={content['home_hero_primary_text'] || "Llamar ahora"}>
              <a
                href={content['home_hero_primary_link'] || "tel:+34691419369"}
              ...
              >
                <Phone size={16} />
                {content['home_hero_primary_text'] || "Llamar ahora"}
              </a>
            </AdminEditable>

            <AdminEditable id="home_hero_secondary_text" content={content['home_hero_secondary_text'] || "Ver catálogo"}>
              <Link
                href={content['home_hero_secondary_link'] || "/catalogo"}
              ...
              >
                <LayoutGrid size={16} />
                {content['home_hero_secondary_text'] || "Ver catálogo"}
              </Link>
            </AdminEditable>
          </div>
        </div>
        ...
        {/* Hero Stats */}
        <div className="max-w-4xl mx-auto py-14 border-t border-white/5 flex flex-col md:flex-row items-center justify-center gap-12 md:gap-24 opacity-0 animate-[fade-in_1s_ease-out_1.2s_forwards]">
          <div className="text-center group cursor-default min-w-[120px]">
            <AdminEditable id="hero-stat-1-num" content={content['hero-stat-1-num'] || "+30"} className="w-full block mb-1">
              <p className="text-5xl font-serif text-gold group-hover:scale-110 transition-transform duration-500 leading-none">{content['hero-stat-1-num'] || "+30"}</p>
            </AdminEditable>
            <AdminEditable id="hero-stat-1-label" content={content['hero-stat-1-label'] || "AÑOS DE HISTORIA"} className="w-full block">
              <p className="text-[10px] uppercase tracking-[0.4em] text-gold/40 font-bold whitespace-nowrap">{content['hero-stat-1-label'] || "AÑOS DE HISTORIA"}</p>
            </AdminEditable>
          </div>

          <div className="w-[1px] h-12 bg-white/5 hidden md:block opacity-20" />

          <div className="text-center group cursor-default min-w-[120px]">
            <AdminEditable id="hero-stat-2-num" content={content['hero-stat-2-num'] || "100%"} className="w-full block mb-1">
              <p className="text-5xl font-serif text-gold group-hover:scale-110 transition-transform duration-500 leading-none">
                {content['hero-stat-2-num']?.includes('%') ? content['hero-stat-2-num'] : `${content['hero-stat-2-num'] || "100"}%`}
              </p>
            </AdminEditable>
            <AdminEditable id="hero-stat-2-label" content={content['hero-stat-2-label'] || "IBÉRICO DE BELLOTA"} className="w-full block">
              <p className="text-[10px] uppercase tracking-[0.4em] text-gold/40 font-bold whitespace-nowrap">{content['hero-stat-2-label'] || "IBÉRICO DE BELLOTA"}</p>
            </AdminEditable>
          </div>
          ...
          <div className="text-center group cursor-default min-w-[120px]">
            <div className="group-hover:scale-110 transition-transform duration-500">
              <AdminEditable id="hero-stat-3-num" content={content['hero-stat-3-num'] || "4.8"} className="w-full block">
                <div className="text-5xl font-serif text-gold leading-none flex items-center justify-center gap-2 mb-1">
                  {content['hero-stat-3-num'] || "4.8"}
                  <Star size={24} className="text-gold fill-gold/10" />
                </div>
              </AdminEditable>
            </div>
            <AdminEditable id="hero-stat-3-label" content={content['hero-stat-3-label'] || "EN GOOGLE"} className="w-full block">
              <p className="text-[10px] uppercase tracking-[0.4em] text-gold/40 font-bold whitespace-nowrap">{content['hero-stat-3-label'] || "EN GOOGLE"}</p>
            </AdminEditable>
          </div>
        </div>
      ...
      {/* 2. TICKER - Separation */}
      <Ticker
        items={content['ticker-items'] ? content['ticker-items'].split(',').map(s => s.trim()) : undefined}
      />
      ...
      <section id="historia" className="py-32 px-6 bg-bg-section border-y border-gold/10 relative overflow-hidden">
        <div className="container mx-auto grid grid-cols-1 lg:grid-cols-2 gap-32 items-center">
          <div className="flex justify-center lg:justify-start">
            <AdminEditable id="about-image" content={content['about-image'] || "/images/local_desde_fuera.jpg"} type="image" className="relative aspect-[4/5] w-full max-w-[500px] block">
              <div className="relative h-full w-full overflow-hidden rounded-[2.5rem] border border-white/5 z-10 bg-bg-base shadow-[0_20px_60px_rgba(0,0,0,0.5)]">
                <Image
                  src={content['about-image'] || "/images/local_desde_fuera.jpg"}
                  alt="La Abacería"
                  fill
          ...
          <div className="relative">
            <AdminEditable id="about-eyebrow" content={content['about-eyebrow'] || "NUESTRA TRADICIÓN"} className="w-full block">
            ...
            <AdminEditable id="about-content" content={content['about-content'] || "En La Abacería no solo vendemos embutidos, seleccionamos historias. Cada pieza ha pasado un riguroso proceso en las mejores dehesas. Nuestro maestro cortador garantiza la temperatura y el grosor exacto en cada loncha."} className="w-full block">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[
                { id: '1', icon: <Scissors size={24} strokeWidth={1.5} />, title: content['about-feature-1-title'] || "Corte a cuchillo", desc: content['about-feature-1-desc'] || "Maestría en cada loncha para preservar todo el aroma." },
                { id: '2', icon: <TreeDeciduous size={24} strokeWidth={1.5} />, title: content['about-feature-2-title'] || "Selección en dehesa", desc: content['about-feature-2-desc'] || "Solo las mejores piezas de bellota llegan aquí." },
                { id: '3', icon: <Ribbon size={24} strokeWidth={1.5} />, title: content['about-feature-3-title'] || "Tradición local", desc: content['about-feature-3-desc'] || "Más de 30 años siendo el referente gourmet." },
                { id: '4', icon: <CheckCircle2 size={24} strokeWidth={1.5} />, title: content['about-feature-4-title'] || "Calidad suprema", desc: content['about-feature-4-desc'] || "Productos con denominación de origen protegida." }
              ].map((feat, idx) => (
                <div key={idx} className="p-10 border border-white/5 rounded-3xl bg-bg-base transition-all hover:bg-bg-base/80 hover:border-gold/20 group">
                  ...
                  <AdminEditable
                    id={`about-feature-${feat.id}`}
                    content={`${feat.title} | ${feat.desc}`}
                    className="w-full block"
                  >
                    {(() => {
                      const cardContent = content[`about-feature-${feat.id}`] || `${feat.title} | ${feat.desc}`
                      const [t, d] = cardContent.split('|').map(s => s.trim())
                      return (
                        <>
                          <h3 className="text-base font-bold text-gold mb-3 tracking-tight">{t || feat.title}</h3>
                          <p className="text-[13px] text-text-muted leading-relaxed font-light opacity-60">{d || feat.desc}</p>
                        </>
                      )
                    })()}
                  </AdminEditable>
      ...
      <section className="py-40 px-6 bg-bg-base relative overflow-hidden">
        ...
        <div className="container mx-auto text-center mb-32">
          <AdminEditable id="baskets-eyebrow" content={content['baskets-eyebrow'] || "EL REGALO PERFECTO"} className="w-full block">
          ...
          {[
            ...
            >
              ...
              <div className="relative h-[430px] overflow-hidden bg-bg-product">
                <AdminEditable id={`baskets-card-${cesta.id}-image`} content={cesta.image} type="image" className="w-full h-full block relative">
                  <Image
                    src={cesta.image}
                    alt={cesta.title}
                    fill
                ...
                <div className="absolute top-8 left-8">
                  <AdminEditable id={`baskets-card-${cesta.id}-badge`} content={cesta.badge}>
              ...
              <div className="flex-grow p-12 flex flex-col justify-between">
                <div>
                  ...
                  <AdminEditable id={`baskets-card-${cesta.id}-text`} content={cesta.desc} className="w-full block">
                ...
                <div className="mt-auto">
                  <div className="flex items-end justify-between mb-8">
                    <div className="font-serif">
                      <AdminEditable id={`baskets-tarifa-${cesta.id}-price`} content={cesta.price}>
      ...
      <section className="py-32 px-6 bg-bg-base relative">
        <div className="container mx-auto mb-20">
          <AdminEditable id="gallery-eyebrow" content={content['gallery-eyebrow'] || "NUESTRO RINCÓN GOURMET"} className="w-full block">
          ...
          <AdminEditable id="gallery-title" content={`${content['gallery-title-left'] || "La tienda, en"} |${content['gallery-title-italic'] || "imágenes"}|`} className="w-full block">
        ...
        <div className="container mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            {
              img: content['gallery-image-1'] || "/images/local_desde_fuera.jpg",
              label: content['gallery-label-1'] || "Nuestra fachada"
            },
            {
              img: content['gallery-image-2'] || "/images/exposicion_vinos.jpg",
              label: content['gallery-label-2'] || "Vinos de selección"
            },
            {
              img: content['gallery-image-3'] || "/images/readme_header.png",
              label: content['gallery-label-3'] || "Corte artesano"
            },
            {
              img: content['gallery-image-4'] || "/images/cristalera_productos_gourmet.jpg",
              label: content['gallery-label-4'] || "Nuestras vitrinas"
            }
          ].map((item, i) => (
            <div key={i} className="group relative aspect-[4/5] rounded-[2.5rem] overflow-hidden border border-white/5 bg-bg-dark shadow-2xl transition-all duration-700 hover:border-gold/30">
              <AdminEditable id={`gallery-image-${i + 1}`} content={item.img} type="image" className="w-full h-full block relative">
              ...
              <div className="absolute bottom-10 left-10 z-10">
                <AdminEditable id={`gallery-label-${i + 1}`} content={item.label} className="w-full block">
        ...
        <div className="container mx-auto">
          <div className="flex flex-col md:flex-row items-center gap-10 md:gap-20 mb-20">
            <div className="flex flex-col items-center">
              <AdminEditable id="reviews-rating" content={content['reviews-rating'] || "4.8"} className="w-full block text-center">
              ...
              <AdminEditable id="reviews-text" content={content['reviews-text'] || "32 reseñas en Google"} className="w-full block text-center">
            ...
            <div className="text-center md:text-left">
              <AdminEditable id="reviews-eyebrow" content={content['reviews-eyebrow'] || "Opiniones"} className="w-full block">
              ...
              <AdminEditable id="reviews-title" content={content['reviews-title'] || "Lo que dicen nuestros clientes"} className="w-full block">
              ...
              <AdminEditable id="reviews-subtitle" content={content['reviews-subtitle'] || "Reseñas verificadas de clientes reales"} className="w-full block">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="p-10 bg-bg-card border border-white/5 rounded-2xl transition-all hover:bg-bg-card/80">
                ...
                <AdminEditable id={`reviews-${review.id}-text`} content={review.text} className="w-full block">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <AdminEditable id={`reviews-${review.id}-author`} content={review.name} className="w-full block">
                      <p className="text-[13px] font-bold text-gold tracking-tight">{review.name}</p>
                    </AdminEditable>
                    <AdminEditable id={`reviews-${review.id}-date`} content={review.date} className="w-full block">
      ...
      <section className="py-32 px-6 bg-bg-base">
        <div className="container mx-auto grid grid-cols-1 lg:grid-cols-12 gap-20 items-center">
          <div className="lg:col-span-5">
            <AdminEditable id="contact-eyebrow" content={content['contact-eyebrow'] || "ESTAMOS EN CORIA"} className="w-full block">
            ...
            <AdminEditable id="contact-title" content={content['contact-title'] || "Visítenos"} className="w-full block">
            ...
            <div className="space-y-10">
              <div className="flex gap-6 items-start">
                ...
                <div>
                  ...
                  <AdminEditable id="contact-address-line1" content={content['contact-address-line1'] || "C. Cervantes, 75"} className="w-full block">
                    <p className="text-gold text-base font-serif">{content['contact-address-line1'] || "C. Cervantes, 75"}</p>
                  </AdminEditable>
                  <AdminEditable id="contact-address-line2" content={content['contact-address-line2'] || "41100 Coria del Río, Sevilla"} className="w-full block">
                ...
                <div className="w-full">
                  ...
                  <div className="space-y-2 max-w-[340px]">
                    <div className="flex justify-between text-[11px]">
                      ...
                      <AdminEditable id="contact-hours-mon-fri" content={content['contact-hours-mon-fri'] || "08:30 - 14:00 | 17:30 - 21:00"}>
                    ...
                    <div className="flex justify-between text-[11px]">
                      ...
                      <AdminEditable id="contact-hours-sat" content={content['contact-hours-sat'] || "08:30 - 14:00"}>
                    ...
                    <div className="flex justify-between text-[11px]">
                      ...
                      <AdminEditable id="contact-hours-sun" content={content['contact-hours-sun'] || "Cerrado"}>
              ...
              <div className="flex gap-6 items-start">
                ...
                <div>
                  ...
                  <AdminEditable id="contact-phone" content={content['contact-phone'] || "+34 691 419 369"} className="w-full block">
              ...
              <div className="flex gap-6 items-start">
                ...
                <div>
                  ...
                  <AdminEditable id="contact-email" content={content['contact-email'] || "info@laabaceriacoria.es"} className="w-full block">
